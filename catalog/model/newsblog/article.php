...
class ModelNewsBlogArticle extends Model {
	...
	public function getArticleCategory($article_id) {
    	...
    	public function getArticleCategorygol($article_id) {
		$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "newsblog_article WHERE article_id = '" . (int)$article_id . "'");
	...
	public function getArticle($article_id) {
		...
		p.sort_order FROM " . DB_PREFIX . "newsblog_article p
	...
	public function getArticles($data = array()) {
		...
		a.status = '1' AND
        a.category_id = '" . (int)$data['filter_category_id'] ."' AND
