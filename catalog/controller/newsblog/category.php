...
class ControllerNewsBlogCategory extends Controller {
	public function index() {
		...
		if (isset($this->request->get['newsblog_category_id'])) {
			$path = '';

			$parts = explode('_', (string)$this->request->get['newsblog_category_id']);
			...
			foreach ($parts as $path_id) {
				...
				if ($category_info) {
					$data['breadcrumbs'][] = array(
						...
						'href' => $this->url->link('newsblog/category', 'newsblog_category_id=' . $path)
					);
				}
			}
		} else {
